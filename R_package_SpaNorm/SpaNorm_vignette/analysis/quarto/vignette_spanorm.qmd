...
3.  Adjustment of data using percentile adjusted counts (PAC) [@Salim2022], as well as other adjustment approaches (e.g., Pearson).
...
head(rownames(HumanDLPFC))
rowData(HumanDLPFC)

# rowData(HumanDLPFC)$gene_name %>% str_subset(pattern = "\\.")
...
The log-transformed raw counts are visualised below for the gene *MOBP* which is a marker of oligodendrocytes enriched in the white matter (WM) [@Maynard2021]. Despite being a marker of this region, we see that it is in fact absent from the white matter region.

```{r fig.width=7.5, fig.height=4.25}
# explor the assays slot
assays(HumanDLPFC)

# populate the logcounts slot.
# this is the default implementaiton in the vignette
# logcounts(HumanDLPFC) <- log2(counts(HumanDLPFC) + 1)

# this can be done also using the assay function
assay(HumanDLPFC, "rawLogCounts") <- log2(counts(HumanDLPFC) + 1) %>%
  # make is as a sparse matrix
...
# add also the plotting usign the library size normalization produced by 
# notice that in the regular workflow this step is achieved by using the function logNormCounts:
# obj <- logNormCounts(obj)
# add the assay using the logNormCounts
HumanDLPFC <- logNormCounts(HumanDLPFC,name = "scaleLogCounts")

# confirm the two processes are not the same as logNormCounts also correct for the library size
assays(HumanDLPFC)
all.equal(assay(HumanDLPFC,i = "rawLogCounts"),assay(HumanDLPFC,i = "scaleLogCounts"))

assay(HumanDLPFC,i = "rawLogCounts") %>% .[1:2,1:2]
assay(HumanDLPFC,i = "scaleLogCounts") %>% .[1:2,1:2]

p_counts2 <- plotSpatial(
    HumanDLPFC,
    colour = MOBP,
    what = "expression",
    assay = "scaleLogCounts",
    size = 0.5
  ) +
  scale_colour_viridis_c(option = "F") +
  ggtitle("scale log counts")

# plot also the raw counts to contextlaize
p_countsRaw <- plotSpatial(
    HumanDLPFC,
    colour = MOBP,
    what = "expression",
    assay = "counts",
...
# plot raw count alone
p_countsRaw

# plot the panel
p_region | p_counts | p_counts2
```
...
```{r message=TRUE}
set.seed(36)
# notice that this will overwrite the logcounts slot if alredy present
HumanDLPFC <- SpaNorm(HumanDLPFC)
HumanDLPFC

assays(HumanDLPFC)

# notice that after the update of SpaNorm, the default assay is now a sparse matrix
# assay(HumanDLPFC,i = "logcounts")
```
...
```{r fig.width=14, fig.height=4.25}
p_logpac <- plotSpatial(
  ...
  scale_colour_viridis_c(option = "F") +
  ggtitle("logPAC")
p_region | p_counts | p_counts2 | p_logpac
```
...
# make sure you are using the version from GitHub
...
The `plotCovariate()` function can be used to explore the learnt functions. We could study what the model has learnt about the biology and library size effects of the *MOBP* gene.
